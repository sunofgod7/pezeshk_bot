...
async function callGeminiWithRetry(requestBody, isMediaRequest = false) {
  ...
  if (isMediaRequest) {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (
              ...
              console.log(`⚠️ کلید ${apiKeyObj.index} با مدل ${model} به Quota خورد، رفتن به مدل بعدی...`);
    ...
    // برای متن: فقط مدل gemini-3.1-flash-lite با تمام کلیدها
    for (let keyIndex = 0; keyIndex < GEMINI_API_KEYS.length; keyIndex++) {
      const apiKeyObj = GEMINI_API_KEYS[keyIndex];
      
      try {
        const GEMINI_URL = `https://generativelanguage.googleapis.com/v1beta/models/${TEXT_MODEL}:generateContent?key=${apiKeyObj.key}`;
        
        console.log(`[Gemini Text] تلاش با کلید ${apiKeyObj.index} و مدل ${TEXT_MODEL}`);
        ...
        if (!response.ok) {
          ...
          // اگر خطای Quota بود، کلید بعدی را امتحان کن
          if (
            errorMsg.includes("quota") ||
            errorMsg.includes("Quota exceeded") ||
            errorMsg.includes("rate limit") ||
            ...
            console.log(`⚠️ کلید ${apiKeyObj.index} به Quota خورد، رفتن به کلید بعدی...`);
            lastError = new Error(errorMsg);
        ...
        // موفقیت
        console.log(`✅ درخواست با کلید ${apiKeyObj.index} موفق بود`);
        return data;
        
      } catch (error) {
        console.error(`❌ خطا با کلید ${apiKeyObj.index}:`, error.message);
        lastError = error;
      }
    }
  }
